...
// Database Enrichment Logic (applied to the raw data)
function enrichSatData(satellites) {
    satellites.forEach((sat, index) => {
        if (!sat.noradId) sat.noradId = 90000 + index;
        if (!sat.altitude) {
            sat.altitude = (sat.orbitType === "GEO" || sat.orbitType === "GSO") ? 35786 : 500;
        }
        const deepSpaceMissions = ["Chandrayaan", "Mars Orbiter", "Aditya", "MOM", "XPoSat"];
        if (deepSpaceMissions.some(m => sat.name.includes(m))) {
            sat.isDeepSpace = true;
        }
        if (sat.inclination === undefined) {
            if (sat.orbitType === "SSO") sat.inclination = 98;
            else if (sat.orbitType === "GEO") sat.inclination = 0;
            else if (sat.orbitType === "GSO") sat.inclination = 15;
            else sat.inclination = 28.5;
        }
        if (!sat.period) {
            if (sat.isDeepSpace) {
                sat.period = 1440 * 10;
        ...
        if ((sat.orbitType === "GEO" || sat.orbitType === "GSO") && sat.geoLongitude === undefined) {
            sat.geoLongitude = 74 + ((index * 2.5) % 20);
        }
...
function getOrbitCategory(sat) {
    ...
    if (sat.altitude >= 30000) {
        if (sat.inclination && sat.inclination > 5) return "GSO";
        return "GEO";
    }
